...
final class ZamkeAudioEngine: ObservableObject {
    ...
    /// 타이핑 라운드 진입 시 — 오디오 엔진 완전 일시정지
    /// 키보드 타이핑 사운드와 겹침 방지
    func pauseForTyping() {
        isSuppressed = true
        // 타이머 모두 정지 (이벤트 스케줄링 차단)
        tickTimer?.invalidate(); tickTimer = nil
        ajaengTimer?.invalidate(); ajaengTimer = nil
        omenTimer?.invalidate(); omenTimer = nil
        zamkeLoopTimer?.invalidate(); zamkeLoopTimer = nil
        ...
        // 모든 플레이어 즉시 정지
        dronePlayer.stop(); ajaengPlayer.stop(); ajaengPlaying = false
        omenPlayer.stop(); omenPlaying = false
        zamkePlayer.stop(); zamkePlaying = false
        moktakPlayer.stop()

        // 엔진 일시정지
        engine.pause()
        #if DEBUG
        print("⏸ 오디오 엔진 일시정지 (타이핑)")
        #endif
    }

    /// 타이핑 라운드 종료 시 — 오디오 엔진 복원 (phase 유지)
    func resumeFromTyping() {
        isSuppressed = false
        do { try engine.start() } catch { print("❌ 엔진 재시작 실패: \(error)"); return }
