...
struct AlarmItem: Identifiable, Codable, Equatable {
    ...
    var snoozeEnabled: Bool = true    // 스누즈 (10분 더, 화들짝 강제)
...
final class AlarmStore: ObservableObject {
    ...
    /// 스누즈 액션 카테고리 ID
    static let snoozeCategoryID = "ZAMKE_ALARM_SNOOZE"
    ...
    func requestPermission() {
        ...
        let snoozeAction = UNNotificationAction(
            identifier: AlarmStore.snoozeActionID,
            title: "10분만 더",
            options: []
        )
        let category = UNNotificationCategory(
            identifier: AlarmStore.snoozeCategoryID,
    ...
    private func scheduleNotification(for alarm: AlarmItem) {
        ...
        if alarm.snoozeEnabled {
            content.categoryIdentifier = AlarmStore.snoozeCategoryID
        }
    ...
    /// 스누즈 알림 예약 (10분 후, 화들짝 강제)
    func scheduleSnoozeNotification() {
        let center = UNUserNotificationCenter.current()
        let content = UNMutableNotificationContent()
        content.title = "ZAMKE"
        content.body = "스누즈 종료 — 이제 일어나야 합니다"
        content.sound = .default
        content.categoryIdentifier = AlarmStore.snoozeCategoryID
        content.userInfo = ["isSnooze": true]

        let trigger = UNTimeIntervalNotificationTrigger(timeInterval: 600, repeats: false)
        let req = UNNotificationRequest(identifier: "zamke_snooze", content: content, trigger: trigger)
        center.add(req)
    }
...
struct AlarmView: View {
    ...
    private var snoozeSection: some View {
        HStack(spacing: 12) {
            ...
            ZamkeToggle(isOn: snoozeEnabled) {
                snoozeEnabled.toggle()
            }
        }
        .padding(.horizontal, 16)
        .padding(.vertical, 12)
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack(spacing: 8) {
                    ...
                    if alarm.snoozeEnabled {
                        ...
                        .foregroundColor(alarm.enabled
                            ? .yellow.opacity(0.6) : textTertiary)
                    }
                }
            }
            .onTapGesture {
